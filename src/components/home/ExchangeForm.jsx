...
function CurrencyDropdown({ options, selected, onSelect }) {
  ...
  useEffect(() => {
    if (!open) return // ✅ ما نربط الـ listener إلا لما تكون مفتوحة

    const handler = (e) => {
      // ✅ نتحقق بعد تأخير صغير عشان الـ React event يكتمل أولاً
      setTimeout(() => {
  ...
  }, [open]) // ✅ يتربط ويتشال مع كل مرة تتغير حالة open

  const handleToggle = useCallback((e) => {
    e.stopPropagation() // ✅ يمنع وصول الـ click للـ document listener
    setOpen(prev => !prev)
  }, [])

  const handleSelect = useCallback((e, currency) => {
    e.stopPropagation()
    onSelect(currency)
    setOpen(false)
  }, [onSelect])
  ...
  return (
    <div ref={ref} style={{ position: 'relative' }}>
      ...
      {/* زر الاختيار */}
      <div
        onClick={handleToggle} // ✅ handleToggle يوقف الانتشار
        style={{
      ...
      >
        ...
        <svg
          ...
          style={{
            opacity: 0.4, transition: 'transform 0.22s',
      ...
      {open && (
        <div
          onClick={(e) => e.stopPropagation()} // ✅ أي click داخل القائمة ما يوصل للـ document
          style={{
            position: 'absolute',
            top: 'calc(100% + 6px)',
            right: 0,
            ...
            zIndex: 9999, // ✅ رفعنا الـ zIndex عشان يطلع فوق أي overlay
            overflow: 'hidden',
            padding: '4px',
          }}
        >
          {options.map(c => (
            ...
            >
              ...
              <span style={{ fontSize: '0.84rem', fontWeight: 700, color: 'var(--text-1)' }}>
                {c.name}
              </span>
              {selected.id === c.id && (
                <svg
                  style={{ marginRight: 'auto', color: 'var(--cyan)', flexShrink: 0 }}
