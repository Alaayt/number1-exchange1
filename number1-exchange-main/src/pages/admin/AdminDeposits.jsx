      ...
      <div style={{
        ...
        <div style={{ padding: '20px 22px 16px', borderBottom: '1px solid var(--border-1)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            ...
            <div style={{ fontSize: '0.68rem', color: 'var(--text-3)', fontFamily: "'JetBrains Mono',monospace", marginTop: 2 }}>{deposit._id}</div>
          </div>
        ...
        <div style={{ padding: 22, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div style={{ background: 'rgba(0,210,255,0.04)', border: '1px solid var(--border-1)', borderRadius: 12, padding: '14px 16px' }}>
            ...
            {[
              ...
              <div key={row.label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '0.85rem', marginBottom: 6 }}>
          ...
          <div style={{ background: 'rgba(0,0,0,0.2)', border: '1px solid var(--border-1)', borderRadius: 12, padding: '14px 16px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {[
                { label: 'المبلغ',  value: `${Number(deposit.amount).toFixed(2)} USDT` },
              ...
              {deposit.txid && (
                <div style={{ marginTop: 4 }}>
                  ...
                  <div style={{
                    fontFamily: "'JetBrains Mono',monospace", fontSize: '0.72rem', color: '#26a17b',
                    wordBreak: 'break-all', background: 'rgba(38,161,123,0.06)', padding: '8px 12px',
                    borderRadius: 8, border: '1px solid rgba(38,161,123,0.2)'
                  }}>
                    {deposit.txid}
                  </div>
                  <a
                    href={`https://tronscan.org/#/transaction/${deposit.txid}`}
                    target="_blank" rel="noreferrer"
                    style={{ display: 'inline-flex', alignItems: 'center', gap: 4, marginTop: 6, fontSize: '0.68rem', color: 'var(--cyan)', textDecoration: 'none', fontFamily: "'JetBrains Mono',monospace" }}
                  >
                    🔗 تحقق على TronScan
                  </a>
                </div>
          ...
          {isPending && !rejectMode && (
            <div style={{ display: 'flex', gap: 10, marginTop: 4 }}>
              <button
                ...
                style={{ flex: 1, padding: '13px 0', background: 'linear-gradient(135deg,#00c97a,#009960)', border: 'none', borderRadius: 12, color: '#fff', fontFamily: "'Tajawal',sans-serif", fontSize: '0.95rem', fontWeight: 800, cursor: loading ? 'not-allowed' : 'pointer', opacity: loading ? 0.7 : 1 }}
              >
                {loading ? '⏳...' : '✅ موافقة وإضافة الرصيد'}
              </button>
              <button
                onClick={() => setRejectMode(true)} disabled={loading}
                style={{ flex: 1, padding: '13px 0', background: 'rgba(248,81,73,0.1)', border: '1px solid rgba(248,81,73,0.3)', borderRadius: 12, color: '#f85149', fontFamily: "'Tajawal',sans-serif", fontSize: '0.95rem', fontWeight: 700, cursor: 'pointer' }}
              >
                ❌ رفض الطلب
              </button>
            </div>
...
export default function AdminDeposits() {
  ...
  const fetchStats = async () => {
    try {
      const [p, a, r, all] = await Promise.all([
        fetch(`${API}/api/admin/deposits?status=pending`,  { headers: { Authorization: `Bearer ${getToken()}` } }).then(r => r.json()),
        fetch(`${API}/api/admin/deposits?status=approved`, { headers: { Authorization: `Bearer ${getToken()}` } }).then(r => r.json()),
        fetch(`${API}/api/admin/deposits?status=rejected`, { headers: { Authorization: `Bearer ${getToken()}` } }).then(r => r.json()),
        fetch(`${API}/api/admin/deposits`,                 { headers: { Authorization: `Bearer ${getToken()}` } }).then(r => r.json()),
      ])
      setStats({ pending: p.pagination?.total || 0, approved: a.pagination?.total || 0, rejected: r.pagination?.total || 0, total: all.pagination?.total || 0 })
    } catch {}
  }

  useEffect(() => { fetchDeposits(); fetchStats() }, [])
  ...
  const handleApprove = async (id) => {
    ...
    showToast(`✅ تمت الموافقة — تم إضافة ${data.balance?.toFixed(2) || ''} USDT`)
  ...
  return (
    <div style={{ padding: '28px 24px', direction: 'rtl', fontFamily: "'Tajawal',sans-serif" }}>
      ...
      {/* Toast */}
      {toast && (
        <div style={{ position: 'fixed', top: 24, left: '50%', transform: 'translateX(-50%)', background: 'var(--card)', border: '1px solid var(--border-2)', borderRadius: 12, padding: '12px 24px', zIndex: 500, fontSize: '0.9rem', color: 'var(--text-1)', boxShadow: '0 8px 32px rgba(0,0,0,0.5)', fontFamily: "'Tajawal',sans-serif", fontWeight: 700 }}>
          {toast}
        </div>
      )}
      ...
      <div style={{ marginBottom: 24 }}>
        <h2 style={{ fontSize: '1.4rem', fontWeight: 900, color: 'var(--text-1)', margin: '0 0 4px' }}>طلبات الإيداع USDT</h2>
        <p style={{ color: 'var(--text-3)', fontSize: '0.82rem', margin: 0 }}>مراجعة وإدارة طلبات إيداع USDT من المستخدمين</p>
      </div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 18, flexWrap: 'wrap' }}>
        {FILTERS.map(f => (
          <button key={f.id} onClick={() => setFilter(f.id)} style={{
            ...
            color: filter === f.id ? f.color : 'var(--text-3)', transition: 'all 0.2s'
          }}>
            {f.label}
          </button>
        ))}
        <button
          ...
          style={{ marginRight: 'auto', padding: '8px 16px', borderRadius: 20, border: '1px solid var(--border-1)', background: 'transparent', color: 'var(--text-3)', cursor: 'pointer', fontSize: '0.82rem', display: 'flex', alignItems: 'center', gap: 6, fontFamily: "'Tajawal',sans-serif" }}
              ...
              <thead>
                <tr style={{ borderBottom: '1px solid var(--border-1)', background: 'rgba(0,0,0,0.15)' }}>
                  {['المستخدم', 'المبلغ', 'TXID', 'الحالة', 'التاريخ', 'إجراء'].map(h => (
                    <th key={h} style={{ padding: '12px 16px', textAlign: 'right', fontSize: '0.68rem', color: 'var(--text-3)', fontWeight: 700, fontFamily: "'JetBrains Mono',monospace", letterSpacing: 1, whiteSpace: 'nowrap' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {deposits.map((d, i) => (
                  ...
                  >
                    ...
                    <td style={{ padding: '13px 16px', fontFamily: "'JetBrains Mono',monospace", fontSize: '0.92rem', fontWeight: 700, color: '#26a17b' }}>
                      {Number(d.amount).toFixed(2)} USDT
                    </td>
                    <td style={{ padding: '13px 16px', fontFamily: "'JetBrains Mono',monospace", fontSize: '0.68rem', color: 'var(--text-3)', maxWidth: 120, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {d.txid || '—'}
                    </td>
                    ...
                    <td style={{ padding: '13px 16px' }} onClick={e => e.stopPropagation()}>
                      <button
                        ...
                        style={{
                          padding: '6px 14px',
                          background: d.status === 'pending' ? 'rgba(0,229,160,0.1)' : 'transparent',
                          border: `1px solid ${d.status === 'pending' ? 'rgba(0,229,160,0.25)' : 'var(--border-1)'}`,
                          borderRadius: 8,
