...
function DepositModal({ isOpen, onClose, onSuccess }) {
  ...
  useEffect(() => {
    ...
    fetch(`${API}/api/wallet/deposit-info`, {
      ...
      .then(r => r.json())
      .then(d => { if (d.success) setDepositInfo(d.depositInfo) })
      .catch(() => {})
      ...
      <div style={{
        ...
        <div style={{ padding: '20px 22px 16px', borderBottom: '1px solid var(--border-1)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{ width: 38, height: 38, borderRadius: 10, background: 'rgba(38,161,123,0.1)', border: '1px solid rgba(38,161,123,0.25)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.1rem' }}>
              ₮
            </div>
            <div>
              <div style={{ fontWeight: 800, fontSize: '1rem', color: 'var(--text-1)' }}>إيداع USDT</div>
        ...
        <div style={{ padding: 20, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{
            ...
            <div style={{ padding: '10px 14px', borderBottom: '1px solid rgba(38,161,123,0.15)', display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            {infoLoading ? (
              <div style={{ padding: '20px', textAlign: 'center', color: 'var(--text-3)', fontSize: '0.82rem' }}>⏳ جاري التحميل...</div>
              ...
              <div style={{ padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 10 }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <span style={{
                    ...
                    color: '#26a17b', background: 'rgba(38,161,123,0.1)', padding: '3px 10px',
                    borderRadius: 20, border: '1px solid rgba(38,161,123,0.2)'
                  }}>
                    {depositInfo.usdtNetwork || 'TRC20'}
                  </span>
                </div>
                ...
                <div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'rgba(0,0,0,0.2)', borderRadius: 10, padding: '10px 12px', border: '1px solid var(--border-1)' }}>
                    <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: '0.72rem', color: '#26a17b', flex: 1, wordBreak: 'break-all', lineHeight: 1.6 }}>
                      {depositInfo.usdtAddress}
                    </span>
                    <CopyBtn value={depositInfo.usdtAddress} />
                  </div>
                </div>
                ...
                <div style={{ padding: '8px 12px', background: 'rgba(248,81,73,0.06)', border: '1px dashed rgba(248,81,73,0.25)', borderRadius: 8, fontSize: '0.75rem', color: '#f85149' }}>
                  🚨 أرسل على شبكة {depositInfo.usdtNetwork || 'TRC20'} فقط — شبكة مختلفة = خسارة الأموال
                </div>

                {depositInfo.note && (
                  <div style={{ padding: '8px 12px', background: 'rgba(245,158,11,0.06)', border: '1px dashed rgba(245,158,11,0.3)', borderRadius: 8, fontSize: '0.75rem', color: 'var(--gold)' }}>
                    📌 {depositInfo.note}
                  </div>
                )}
              </div>
              ...
              <div style={{ padding: '16px', textAlign: 'center' }}>
                <div style={{ fontSize: '0.82rem', color: 'var(--text-3)' }}>⚠️ لم يتم إعداد عنوان USDT بعد</div>
                <div style={{ fontSize: '0.72rem', color: 'var(--text-3)', marginTop: 4 }}>تواصل مع الدعم للحصول على عنوان المحفظة</div>
              </div>
            )}
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: 10, alignItems: 'end' }}>
            ...
            <div style={{ padding: '12px 18px', background: 'rgba(38,161,123,0.08)', border: '1px solid rgba(38,161,123,0.25)', borderRadius: 12, fontFamily: "'JetBrains Mono',monospace", fontWeight: 700, color: '#26a17b', fontSize: '0.9rem' }}>
              USDT
            </div>
          </div>
          ...
          <div>
            <label style={{ display: 'block', fontSize: '0.7rem', color: 'var(--text-3)', marginBottom: 8, fontWeight: 700, fontFamily: "'JetBrains Mono',monospace", letterSpacing: 1 }}>
              رقم المعاملة (TXID) — من محفظتك بعد الإرسال
            </label>
            <input
          ...
          <button
            ...
            style={{ width: '100%', padding: 14, background: loading ? 'rgba(38,161,123,0.3)' : 'linear-gradient(135deg,#26a17b,#1a7a5e)', border: 'none', borderRadius: 12, fontFamily: "'Tajawal',sans-serif", fontSize: '1rem', fontWeight: 800, color: '#fff', cursor: loading ? 'not-allowed' : 'pointer', transition: 'all 0.3s' }}
          >
...
// ══════════════════════════════════════════════
// مودال السحب — تواصل عبر واتساب أو تيليجرام
// ══════════════════════════════════════════════
function WithdrawModal({ isOpen, onClose, balance }) {
  const [amount,       setAmount]       = useState('')
  const [method,       setMethod]       = useState('moneygo') // moneygo | usdt
    ...
    >
      <div style={{
        ...
        <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div>
            <label style={{ display: 'block', fontSize: '0.7rem', color: 'var(--text-3)', marginBottom: 8, fontWeight: 700, fontFamily: "'JetBrains Mono',monospace", letterSpacing: 1 }}>
              المبلغ المراد سحبه (USDT)
            </label>
            <input
              type="number" value={amount} min="1" max={balance}
              onChange={e => setAmount(e.target.value)}
              placeholder="0.00"
              style={{ width: '100%', padding: '12px 14px', background: 'rgba(255,255,255,0.04)', border: '1px solid var(--border-1)', borderRadius: 12, color: 'var(--text-1)', fontFamily: "'JetBrains Mono',monospace", fontSize: '1rem', boxSizing: 'border-box', outline: 'none', direction: 'ltr' }}
            ...
            {amount && parseFloat(amount) > balance && (
              <div style={{ marginTop: 6, fontSize: '0.72rem', color: '#f85149', fontFamily: "'Tajawal',sans-serif" }}>
          ...
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              {[
                { id: 'moneygo', label: 'MoneyGo USD', icon: '💳' },
                ...
                <button key={opt.id} onClick={() => setMethod(opt.id)} style={{
                  ...
                  border: method === opt.id ? '1px solid var(--cyan)' : '1px solid var(--border-1)',
                  background: method === opt.id ? 'rgba(0,210,255,0.08)' : 'rgba(0,0,0,0.15)',
                  color: method === opt.id ? 'var(--cyan)' : 'var(--text-2)',
          ...
          {amountValid && (
            <div style={{ background: 'rgba(0,210,255,0.04)', border: '1px solid rgba(0,210,255,0.15)', borderRadius: 12, padding: '12px 14px' }}>
              <div style={{ fontSize: '0.68rem', color: 'var(--text-3)', fontFamily: "'JetBrains Mono',monospace", marginBottom: 8, letterSpacing: 1 }}>
                الرسالة التي ستُرسل للأدمن
              </div>
              <div style={{ fontSize: '0.85rem', color: 'var(--text-1)', fontFamily: "'Tajawal',sans-serif", lineHeight: 1.6, background: 'rgba(0,0,0,0.2)', padding: '10px 12px', borderRadius: 8 }}>
                {buildMessage()}
              </div>
            </div>
          )}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            <div style={{ fontSize: '0.72rem', color: 'var(--text-3)', textAlign: 'center', fontFamily: "'Tajawal',sans-serif" }}>
              اختر طريقة التواصل مع الأدمن
            </div>
            ...
            {infoLoading ? (
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                ...
                <button
                  ...
                  disabled={!amountValid || !withdrawInfo?.whatsapp}
                  style={{
                    padding: '13px 16px', borderRadius: 12, cursor: amountValid && withdrawInfo?.whatsapp ? 'pointer' : 'not-allowed',
                    fontFamily: "'Tajawal',sans-serif", fontSize: '0.9rem', fontWeight: 700,
                    background: amountValid && withdrawInfo?.whatsapp ? 'rgba(37,211,102,0.1)' : 'rgba(255,255,255,0.03)',
                    border: `1px solid ${amountValid && withdrawInfo?.whatsapp ? 'rgba(37,211,102,0.3)' : 'var(--border-1)'}`,
                    color: amountValid && withdrawInfo?.whatsapp ? '#25d366' : 'var(--text-3)',
                    display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8,
                    transition: 'all 0.2s', opacity: amountValid ? 1 : 0.5
                  }}
                >
                ...
                <button
                  ...
                  disabled={!amountValid || !withdrawInfo?.telegram}
                  style={{
                    padding: '13px 16px', borderRadius: 12, cursor: amountValid && withdrawInfo?.telegram ? 'pointer' : 'not-allowed',
                    fontFamily: "'Tajawal',sans-serif", fontSize: '0.9rem', fontWeight: 700,
                    background: amountValid && withdrawInfo?.telegram ? 'rgba(0,136,204,0.1)' : 'rgba(255,255,255,0.03)',
                    border: `1px solid ${amountValid && withdrawInfo?.telegram ? 'rgba(0,136,204,0.3)' : 'var(--border-1)'}`,
                    color: amountValid && withdrawInfo?.telegram ? '#0088cc' : 'var(--text-3)',
                    display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8,
                    transition: 'all 0.2s', opacity: amountValid ? 1 : 0.5
                  }}
                >
            ...
            {!amountValid && (
              <div style={{ textAlign: 'center', fontSize: '0.72rem', color: 'var(--text-3)', fontFamily: "'Tajawal',sans-serif" }}>
                أدخل مبلغاً صحيحاً أولاً لتفعيل أزرار التواصل
              </div>
            )}
          </div>
        </div>
  ...
  return (
    <div style={{ minHeight: '80vh', padding: '50px 24px 80px', maxWidth: 720, margin: '0 auto', direction: 'rtl' }}>
      ...
      <div style={{
        ...
        border: '1px solid rgba(0,212,255,0.2)', borderRadius: 20, padding: '28px 24px',
        ...
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginBottom: 22 }}>
          <span style={{ fontSize: '3rem', fontWeight: 900, color: '#26a17b', fontFamily: "'Orbitron',sans-serif", lineHeight: 1 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          <button
            ...
            style={{
              padding: 13, background: 'linear-gradient(135deg,#26a17b,#1a7a5e)',
              border: 'none', borderRadius: 12, color: '#fff',
              fontFamily: "'Tajawal',sans-serif", fontSize: '0.95rem', fontWeight: 800,
              cursor: wallet?.isActive ? 'pointer' : 'not-allowed',
              opacity: wallet?.isActive ? 1 : 0.5, transition: 'all 0.2s',
          ...
          <button
            ...
            disabled={!wallet?.isActive || !wallet?.balance || wallet?.balance <= 0}
            style={{
              padding: 13,
              background: wallet?.balance > 0 ? 'linear-gradient(135deg,#009fc0,#006e9e)' : 'rgba(255,255,255,0.05)',
              border: 'none', borderRadius: 12,
              color: wallet?.balance > 0 ? '#fff' : 'var(--text-3)',
              fontFamily: "'Tajawal',sans-serif", fontSize: '0.95rem', fontWeight: 800,
        ...
        {!wallet?.isActive && (
          <div style={{ marginTop: 12, textAlign: 'center', fontSize: '0.78rem', color: '#f85149', fontFamily: "'Tajawal',sans-serif" }}>
        ...
        <div style={{ display: 'flex', borderBottom: '1px solid var(--border-1)' }}>
          {[
            ...
            <button key={tab.id} onClick={() => setActiveTab(tab.id)} style={{
              ...
              fontWeight: activeTab === tab.id ? 700 : 400, cursor: 'pointer', transition: 'all 0.2s'
            }}>
              {tab.label}
            </button>
          ))}
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={{ borderBottom: '1px solid var(--border-1)' }}>
                    {['المبلغ', 'TXID', 'الحالة', 'التاريخ'].map(h => (
                ...
                <tbody>
                  {deposits.map((d, i) => (
                    <tr key={d._id} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)', background: i % 2 === 0 ? 'transparent' : 'rgba(255,255,255,0.01)' }}>
                      <td style={{ padding: '12px 14px', fontFamily: "'JetBrains Mono',monospace", fontSize: '0.9rem', fontWeight: 700, color: '#26a17b' }}>
                        {Number(d.amount).toFixed(2)} USDT
                      </td>
                      <td style={{ padding: '12px 14px', fontFamily: "'JetBrains Mono',monospace", fontSize: '0.68rem', color: 'var(--text-3)', maxWidth: 120, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                        {d.txid || '—'}
                      </td>
