...
router.post('/', async (req, res) => {
  try {
    ...
    if (!moneygo.recipientName || !moneygo.recipientPhone || !moneygo.amountUSD) {
      return res.status(400).json({
        success: false,
        message: 'Recipient name, phone, and amount are required.'
      });
    }
    ...
    try {
      ...
      // لا نوقف الطلب بسبب فشل التليغرام
    }

    // ─── إرسال صورة الإيصال (للمحافظ المصرية) ─
...
// ─── GET /api/orders/my ───────────────────────
// طلبات المستخدم الحالي (يجب تسجيل الدخول)
...
// التحقق من معاملة USDT (العميل يُرسل الـ txHash)
